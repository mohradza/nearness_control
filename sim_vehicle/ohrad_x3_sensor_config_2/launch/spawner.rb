def spawner(_name, _modelURI, _worldName, _x, _y, _z, _roll, _pitch, _yaw)
  ...
  <spawn name='#{_name}'>
      ...
      <pose>#{_x} #{_y-5.0} #{_z + 0.2} #{_roll} #{_pitch} #{_yaw}</pose>
...
def rosExecutables(_name, _worldName)
  <<-HEREDOC
    <executable name='x3_description'>
      <command>roslaunch --wait ohrad_x3_sensor_config_2 description.launch world_name:=#{_worldName} name:=#{_name}</command>
    </executable>
    <executable name='x3_ros_ign_bridge'>
      <command>roslaunch --wait ohrad_x3_sensor_config_2 vehicle_topics.launch world_name:=#{_worldName} name:=#{_name} uav:=1 laser_scan:=0 top_scan:=1 bottom_scan:=1 rgbd_cam:=1    </command>
  </executable>
  HEREDOC
end
