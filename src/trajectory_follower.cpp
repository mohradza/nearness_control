...
void trajectoryFollower::findNextLookahead(){
    ...
    for (int i = last_lookahead_index_; i < traj_list_size_ - last_lookahead_index_; i++){
        if(dist(odom_point_, traj_list_points_[i]) > lookahead_dist_){
            lookahead_point_ = traj_list_points_[i];
